...
const struct m_sub_options mp_subtitle_sub2_opts = {
    .opts = (const struct m_option[]){
            {"secondary-sub-delay", OPT_FLOAT(sub_delay)},
            {"secondary-sub-fps", OPT_FLOAT(sub_fps)},
            {"secondary-sub-speed", OPT_FLOAT(sub_speed)},
            {"secondary-sub-visibility", OPT_BOOL(sub_visibility)},
            {"secondary-stretch-dvd-subs", OPT_BOOL(stretch_dvd_subs)},
            ...
            {"secondary-sub-pos", OPT_INT(sub_pos), M_RANGE(0, 150)},
            {"secondary-sub-gauss", OPT_FLOAT(sub_gauss), M_RANGE(0.0, 3.0)},
            {"secondary-sub-gray", OPT_BOOL(sub_gray)},
            {"secondary-sub-ass", OPT_BOOL(ass_enabled), .flags = UPDATE_SUB_HARD},
            {"secondary-sub-scale", OPT_FLOAT(sub_scale), M_RANGE(0, 100)},
            {"secondary-sub-ass-line-spacing", OPT_FLOAT(ass_line_spacing),
                    M_RANGE(-1000, 1000)},
            {"secondary-sub-use-margins", OPT_BOOL(sub_use_margins)},
            {"secondary-sub-ass-force-margins", OPT_BOOL(ass_use_margins)},
            {"secondary-sub-ass-vsfilter-aspect-compat", OPT_BOOL(ass_vsfilter_aspect_compat)},
            {"secondary-sub-ass-vsfilter-color-compat", OPT_CHOICE(ass_vsfilter_color_compat,
                                                             {"no", 0}, {"basic", 1}, {"full", 2}, {"force-601", 3})},
            ...
            {"secondary-sub-ass-styles", OPT_STRING(ass_styles_file),
                    .flags = M_OPT_FILE | UPDATE_SUB_HARD},
            {"secondary-sub-ass-hinting", OPT_CHOICE(ass_hinting,
                                                     {"none", 0}, {"light", 1}, {"normal", 2}, {"native", 3})},
            {"secondary-sub-ass-shaper", OPT_CHOICE(ass_shaper,
                                                    {"simple", 0}, {"complex", 1})},
            {"secondary-sub-ass-justify", OPT_BOOL(ass_justify)},
            {"secondary-sub-ass-override", OPT_CHOICE(ass_style_override,
                                                      {"no", 0}, {"yes", 1}, {"force", 3}, {"scale", 4}, {"strip", 5})},
            {"secondary-sub-scale-by-window", OPT_BOOL(sub_scale_by_window)},
            {"secondary-sub-scale-with-window", OPT_BOOL(sub_scale_with_window)},
            {"secondary-sub-ass-scale-with-window", OPT_BOOL(ass_scale_with_window)},
            {"secondary-sub", OPT_SUBSTRUCT(sub_style, sub2_style_conf)},
            {"secondary-sub-clear-on-seek", OPT_BOOL(sub_clear_on_seek)},
...
// ------------------------- subtitles options --------------------
    ...
    {"", OPT_SUBSTRUCT(subs2_rend, mp_subtitle_sub2_opts)},
